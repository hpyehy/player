...
//进行这些操作的时候要去掉一些不能设置的字段
enum {
	SQL_INSERT = 1,//INSERT INTO
	SQL_MODIFY = 2,//UPDATE SET
	SQL_CONDITION = 4//WHERE
};
...
enum {
	...
	DEFAULT = 2,
	UNIQUE = 4,
	PRIMARY_KEY = 8,//if (field->Attr & PRIMARY_KEY) sql += "PRIMARY KEY";
	CHECK = 16,
	AUTOINCREMENT = 32
};

//用于表示字段的类型，与数据库中的类型字符串对应，如 TYPE_VARCHAR 对应 VARCHAR(100)
using SqlType = enum {
	TYPE_NULL = 0,
	TYPE_BOOL = 1,
	TYPE_INT = 2,
