...
public:
		...
		m_thread(&CLoggerServer::ThreadFunc, this)
	{
		m_server = NULL;
		char curpath[256] = "";
		getcwd(curpath, sizeof(curpath));//获取当前进程的工作目录(当前路径),如/home/user/project
		m_path = curpath;
...
public:
	...
	int Start() {
		...
		//把本地服务端套接字加入epoll作为监听 socket
		ret = m_epoll.Add(*m_server, EpollData((void*)m_server), EPOLLIN | EPOLLERR);
		if (ret != 0) {
			Close();
			return -6;
		}
		ret = m_thread.Start();//执行线程绑定的函数，即ThreadFunc( 这里不使用线程池)，也就是说日志服务器是单线程的，只启动了一个日志处理线程
		if (ret != 0) {
	...
	//这里两个函数才是功能函数，给其他非日志进程的进程和线程使用的
	//根据别的进程(客户端任务)发送来的内容，将对应的客户端加入 epoll监听，然后让
	static void Trace(const LogInfo& info) {
		int ret = 0;
		static thread_local CSocket client;
		//第一次要连接服务器，加入epoll监听，之后直接把数据发送给服务端的 ThreadFunc 处理
		if (client == -1) {
