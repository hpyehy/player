...
public:
	...
	virtual int Init(const CSockParam& param) {
		if (m_status != 0) return -1;  // 如果 Socket 已初始化，返回 -1，避免重复初始化
		m_param = param;  // 复制 Socket 参数（IP、端口、属性）
		// 确定 Socket 类型：UDP (SOCK_DGRAM) 或 TCP (SOCK_STREAM)
		int type = (m_param.attr & SOCK_ISUDP) ? SOCK_DGRAM : SOCK_STREAM;

		// 创建 本地 Socket
		if (m_socket == -1) {  // 仅在 Socket 还未创建的情况下执行
			if (param.attr & SOCK_ISIP)
				m_socket = socket(PF_INET, type, 0);  // 创建 IPv4 Socket
			else
				m_socket = socket(PF_LOCAL, type, 0); // 创建本地 Unix 域 Socket
		}
		else {
			m_status = 2;  // 如果 Socket 不是 -1，说明是 accept() 返回的 Socket，直接标记为已连接
		}
		...
		// 服务器模式
		if (m_param.attr & SOCK_ISSERVER) {  // 判断是否是服务器 Socket
			if (param.attr & SOCK_ISIP)
				ret = bind(m_socket, m_param.addrin(), sizeof(sockaddr_in));  // 绑定 IPv4 Socket
			else
				ret = bind(m_socket, m_param.addrun(), sizeof(sockaddr_un));  // 绑定本地 Unix 域 Socket

			if (ret == -1) return -3;  // bind() 失败，返回 -3（端口占用，路径已存在）
			// 监听 Socket（仅适用于 TCP，UDP 无需 listen()）
			ret = listen(m_socket, 32);  // 监听最多 32 个 Socket 连接
			if (ret == -1) return -4;  // listen() 失败，返回 -4（资源不足）
		}
		// 非阻塞模式配置（如果 Socket 设置为非阻塞）
		if (m_param.attr & SOCK_ISNONBLOCK) {
			int option = fcntl(m_socket, F_GETFL);  // 获取 Socket 当前状态
			if (option == -1) return -5;  // fcntl() 失败，返回 -5

			option |= O_NONBLOCK;  // 设置 O_NONBLOCK 使 Socket 变为非阻塞
			ret = fcntl(m_socket, F_SETFL, option);  // 应用新配置
			if (ret == -1) return -6;  // fcntl() 设置失败，返回 -6
		}
		if (m_status == 0)
			m_status = 1;
		return 0;
	...
	virtual int Link(CSocketBase** pClient = NULL) {
		...
		if (m_param.attr & SOCK_ISSERVER) {
			...
			if (m_param.attr & SOCK_ISIP) {
				...
				//第二个参数是一个指向 struct sockaddr 的指针，从外部传入，accept() 会将新连接的客户端的地址信息存储在这里
				//fd是accept生成的专门用于和客户端通信的文件描述符
			...
			if (fd == -1)return -3;
			*pClient = new CSocket(fd);// 创建新的连接对象
			if (*pClient == NULL)return -4;
			ret = (*pClient)->Init(param);// 用 param 初始化连接状态
			if (ret != 0) {
				delete (*pClient);
				*pClient = NULL;
				return -5;
