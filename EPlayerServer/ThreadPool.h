...
public:
	int Start(unsigned count) {
		...
		//这里 m_server 是本地套接字，之后 AddTask 和 TaskDispatch 函数中的 Send 和 Recv 都在使用
	...
	int AddTask(_FUNCTION_ func, _ARGS_... args) {
		//目的是 连接服务器后向服务端 (即线程中的threadFunc函数) 发送要执行的函数信息
		static thread_local CSocket client;//thread_local 表示变量是线程局部的，每个线程有自己独立的实例
		int ret = 0;
		if (client == -1) {
			ret = client.Init(CSockParam(m_path, 0));
			if (ret != 0)return -1;
			ret = client.Link();//本地套接字连接服务器，这样服务器 m_server.m_socket 会收到客户端连接信息，epoll会被唤醒，然后将客户端加入epoll监听
			if (ret != 0)return -2;
		}
